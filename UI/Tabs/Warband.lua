...
local function warbandBuilder(parent)
    ...
    local catDD = QOL.CreateChoice(body, 170, 24, QOL.WarbandProps.CategoryNames(), "category")
    catDD:SetPoint("TOPLEFT", body, "TOPLEFT", 384, -(8 + hdr:GetHeight() + 8))

    local propDD = QOL.CreateChoice(body, 170, 24, {}, "prop")
    ...
    local placeBtn = QOL.MakeFlatButton(body, 110, 24, "Place", { justify = "CENTER" })
    placeBtn:SetPoint("TOPLEFT", catDD, "BOTTOMLEFT", 0, -10)
    local removeBtn = QOL.MakeFlatButton(body, 140, 24, "Remove nearest", { justify = "CENTER" })
